package theRavenFileIO;

import java.io.*;
import java.util.*;
import java.util.Map.Entry;

public class TheRavenGuideAttempt {
	...
	public static Map<String, Integer> countTheWords(String fileName) {
		...
		try {
			...
			while (fileReader.hasNext()) { // while there is stuff in the file, keep going
				...
				//poemString = poemString.replaceAll("\\W+", " "); //turn anything not a word into a space
				...
				poemString = poemString.replaceAll("[^A-Za-z0-9 ]", " "); // remove all non-word characters excluding space
				...
				StringTokenizer tokenString = new StringTokenizer(poemString, " "); //this will allow the removal of html code
	...
	public static List<Entry<String, Integer>> sortValue(Map<String, Integer> mapWords) {
		...
		// Collections.sort(theList, Collections.reverseOrder()); <- this caused a fatal error
